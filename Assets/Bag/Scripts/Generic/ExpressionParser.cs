...
namespace B83.ExpressionParser
{
	...
	public class ExpressionParser
	{
		...
		public ExpressionParser()
		{
			...
			m_Funcs.Add("lerp", (p) => { return (1 - p.ElementAtOrDefault(2)) * p.FirstOrDefault() + p.ElementAtOrDefault(2) * p.ElementAtOrDefault(1); }); // added by berghy
			m_Funcs.Add("lerpinv", (p) => { return (p.ElementAtOrDefault(2) - p.FirstOrDefault()) / (p.ElementAtOrDefault(1) - p.FirstOrDefault()); }); // added by berghy
			m_Funcs.Add("clamp", (p) => UnityEngine.Mathf.Clamp((float)p.FirstOrDefault(), (float)p.ElementAtOrDefault(1), (float)p.ElementAtOrDefault(2)));
		...
		IValue Parse(string aExpression)
		{
			...
			if(index2a >= 0 && index2b >= 2)
			{
				...
				int bracketIndex;
				if(int.TryParse(inner, System.Globalization.NumberStyles.Integer, Bag.Unity.ParseCulture, out bracketIndex) && bracketIndex >= 0 && bracketIndex < m_BracketHeap.Count)
				{
			...
			double doubleValue;
			if(double.TryParse(aExpression, System.Globalization.NumberStyles.Float | System.Globalization.NumberStyles.AllowThousands, Bag.Unity.ParseCulture, out doubleValue))
			{
